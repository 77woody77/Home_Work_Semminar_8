...
def show_contacts(book: dict, message: str ):
    ...
    if book:
        for i, contact in book.items():
            print(f'{i:>3}. {contact[0]:<30} {contact[1]:<20} {contact[2]:<20}')
    else:
        print(message)
    print('='*67 + '\n')
...
def change_contact(book: dict, cid: int):
    ...
    ch = []
    for item in ['Введите новое имя (или оставьте пустым чтоб не изменять): ',
                 'Введите новый телефон (или оставьте пустым чтоб не изменять):  ',
                 'Введите новый комментарий (или оставьте пустым чтоб не изменять):  ']:
